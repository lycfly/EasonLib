...
class restoring_div(SIZEINA: Int, SIZEINB: Int) extends Component {
  ...
  io.remainder := (rem_sign_out ? (~p_remainder(PR_DW - 1 downto SIZEINA).asSInt + S(1)) | p_remainder(PR_DW - 1 downto SIZEINA).asSInt)
  io.quot := div_zero_flag ? S((SIZEINA downto 0) -> True) |
            (quot_sign_out ? (B(1, 1 bits) ## ((~quot_reversed).asUInt + 1).asBits).asSInt | (B(0, 1 bits) ## quot_reversed).asSInt)
      ...
      val monitor_thread = fork {
        for (c <- (-A_bound) to (A_bound - 1)) {
          for (d <- (-B_bound) to (B_bound - 1)) {
            if (d == 0) {
              dut.clockDomain.waitSamplingWhere(dut.io.dout_vld.toBoolean)
              true_quot = dut.io.quot.toInt & (pow(2, A).toInt - 1)
              true_rem = dut.io.remainder.toInt & (pow(2, B).toInt - 1)
//              assert((pow(2, B).toInt - 1) == true_quot, "Quot data Mismatch")
//              assert((c) == true_rem, "Reminder data Mismatch")
              println(s"PASSED! Input A = ${c}, B = ${d}. DUT Quot = ${true_quot}; Expect Quot = ${pow(2, B).toInt - 1}, DUT Rem = ${true_rem}; Expect Rem = ${c}")
            }
            else {
              dut.clockDomain.waitSamplingWhere(dut.io.dout_vld.toBoolean)
              // quot
