...
class booth4 (SIZEINA: Int, SIZEINB:Int) extends Component {
 ...
 // val dinBReg = Reg(SInt (SIZEINA bits)) init(0)
  assert(SIZEINA % 2 == 0, "booth input A must be 2*N bitwidth!")
...
object booth4_inst {
  def main(args: Array[String]): Unit = {
    SpinalConfig(
      ...
      mode=Verilog,
      targetDirectory="rtl"
      ).generate(new booth4(SIZEINA = 20, SIZEINB = 10))
...
object b4_inst_syn {
  def main(args: Array[String]): Unit = {
    ...
    val dc = new DesignCompilerFlow(
      design = new booth4(SIZEINA = 8, SIZEINB = 8),
      topModuleName = "booth4",
      workspacePath = "/home/lyc/projects/kws/tiny_vector_core/syn/booth4",
      DCConfig = dc_config,
      designPath = ""
    ).doit()
  }
